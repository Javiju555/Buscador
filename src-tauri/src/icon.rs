...
#[cfg(target_os = "linux")]
static LAUNCHER_ICON_INDEX: OnceLock<Mutex<HashMap<String, PathBuf>>> = OnceLock::new();

#[cfg(target_os = "linux")]
static INDEX_INITIALIZED: OnceLock<std::sync::Arc<std::sync::atomic::AtomicBool>> = OnceLock::new();

#[cfg(target_os = "linux")]
pub fn lazy_init_icon_index() {
...
fn search_themed_icon(icon: &str) -> Option<PathBuf> {
    ...
    if let Some(is_init) = INDEX_INITIALIZED.get() {
        if is_init.load(std::sync::atomic::Ordering::Relaxed) {
            if let Some(index_guard) = LAUNCHER_ICON_INDEX.get() {
                if let Ok(guard) = index_guard.lock() {
    ...
    for ext in &["png", "svg", "jpg", "jpeg"] {
        // Flat pixmaps
        let pixmap_path = PathBuf::from(format!("/usr/share/pixmaps/{icon}.{ext}"));
